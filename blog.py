...
#renders pages using jinja2
def render_str(template, **params):
    t = jinja_env.get_template(template)
    return t.render(params)

#generates a cookie hash
def make_secure_val(val):
    return '%s|%s' % (val, hmac.new(secret, val).hexdigest())

#Checks if its a secure hash and returns the value if legit
...
class BlogHandler(webapp2.RequestHandler):
    ...
    #Sets a cookie whose name is name and value is val. Sets the cookie. Doesn't have an expire time so it will expire when the browser closes by default
    ...
    #given the name, if the cookie exists and passes checksecure value, it will return the cookie name
    ...
    # checks the cookie
    #check for the user cookie (user_id) when the blog starts. Its for keeping track of which users are doing what.
...
# user stuff
#makes a random string of 5 lettters
def make_salt(length=5):
    return ''.join(random.choice(letters) for x in xrange(length))


# h is what we store in the db
#Takes a user name and password and returns the salt, hash
...
#Takes the name, password, and hash and make sure the hash from the database matches the user hash
...
# in our db
#For user groups. Creates ancestor element in database to store our parameters.
...
#inherits from the Signup header.
class Register(Signup):
    #overrides the sign up class so when done is called, this method is called.
    def done(self):
...
class Post(db.Model):
    ...
    #keeps track of who liked the post so only one like from each person
...
class UpdatePost(BlogHandler):
    def get(self, post_id):
        if not self.user:
            ...
            post = db.get(key)
            if not post:
                self.write("the post doesn't exist")
                return
            n1 = post.created_by
            n2 = self.user.name
            print "n1 = ", n1
...
class CommentError(BlogHandler):
    def get(self):
        self.write('Error: You can not edit or delete posts that are not yours.')
